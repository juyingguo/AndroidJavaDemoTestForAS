...
public class NetUtils {
     ...
     * @see #checkUnusefulNetworkByInetAddress()
     */
    public synchronized  static boolean checkUnusefulNetworkByPing(Context context, String[] serverAddresses) {
    ...
    public synchronized  static boolean checkUnusefulNetworkByInetAddress() {
        ...
        for (String serverAddress : SERVER_ADDRESSES){
            LogUtil.e(TAG, TAG + ">>>>serverAddress>>>IOException:" + serverAddress);
            try {
...
//        System.out.println(">>>>checkNetworkByAccess>>>consume time:" + (System.currentTimeMillis() - beginTime) + ",result:" + result);
    ...
    private static boolean isAddressCanConnected(String urlStr) {
        System.out.println(">>>isAddressCanConnected（）>>>urlStr:" + urlStr);
        boolean result = false;
        long beginTime  = System.currentTimeMillis();
        URL url = null;
...
//            conn.connect();
        ...
        EventBus.getDefault().post(new MessageEvent((System.currentTimeMillis() - beginTime) + ""));
        return result;
    }
