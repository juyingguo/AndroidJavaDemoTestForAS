 ...
 *
 * 20201230 copy form {@link CaptureActivityHandler},Change the fixed activity(CaptureActivity) to a common base class or interface {@link DecodeInterface}
 * @author dswitkin@google.com (Daniel Switkin)
 */
public final class BaseCaptureActivityHandler extends Handler {

  private static final String TAG = BaseCaptureActivityHandler.class.getSimpleName();

  private final DecodeInterface activity;
  private final BaseDecodeThread decodeThread;
  private State state;
  ...
  public BaseCaptureActivityHandler(DecodeInterface activity, Vector<BarcodeFormat> decodeFormats,
                                    String characterSet) {
    this.activity = activity;
    decodeThread = new BaseDecodeThread(activity, decodeFormats, characterSet,
  ...
  public void handleMessage(Message message) {
    switch (message.what) {
      ...
      case MessageIDs.return_scan_result:
        ...
        activity.mySetResult(Activity.RESULT_OK, (Intent) message.obj);
        /*activity.finish()*/;
        activity.myFinish();
        break;
