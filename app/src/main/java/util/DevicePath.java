                ...
                for (String path : pathes) {
                    if (!TextUtils.isEmpty(path) && new File(path).exists()) {
                        ...
                        if (statFs.getBlockCount() * statFs.getBlockSize() != 0) {
                            ...
                            LogUtil.d(TAG, ">>>>>>getStoragePaths()>>>path(存储空间不为空):" + path);
                        }else {
                            LogUtil.d(TAG, ">>>>>>getStoragePaths()>>>path(存储空间 == 0):" + path);
                        }
                    }
                }
            }
...
//		return pathsList.toArray(new String[pathsList.size()]);
        LogUtil.e(TAG,"yison all list "+pathsList);
        return pathsList;
    }
    ...
    public String getUsbStoragePath(Context context) {
        String path = "";
        totalDevicesList = (ArrayList<String>) getStoragePaths(context);
        LogUtil.e(TAG,"yison totalDevicesList "+totalDevicesList);
        for (int i = 0; i < totalDevicesList.size(); i++) {
            if (!totalDevicesList.get(i).equals(Environment.getExternalStorageDirectory().getPath())) {
                if (totalDevicesList.get(i).equals("uhost")) {
    ...
    public boolean isExistUsbStoragePath(Context context) {
        boolean isExitUsb = !TextUtils.isEmpty(getUsbStoragePath(context));
        LogUtil.d(TAG, "yison >>>>>>isExistUsbStoragePath()>>>:" + isExitUsb);
		/*ThreadUtils.runOnUiThread(new Runnable() {
			
			@Override
			public void run() {
				ToastUtil.show(MyApplication.getInstance(), "" + TextUtils.isEmpty(getUsbStoragePath()), 1);
